...
var c, define_pipeline_material_das_particulas, emitir, gr, job, ls, main, mat, pipe_tex, pipe_tex_additive, prepara_html, prepara_instancias, prepara_pipelines, prepara_shader_groups, ps, renderiza, res, tema, upd;
...
main = async() => {
  ...
  res = c.resources_from_files('shader_tex.wgsl', 'data/pp[01-46].png');
  await res.load_all();
  prepara_pipelines();
...
prepara_instancias = function() {
  ...
  ls.use_groups({
    global_index: 0,
    global_group: gr.global,
    material_index: 1,
    instance_index: 2,
    instance_group: gr.instancias
  });
  ps = c.particle_system();
  return define_pipeline_material_das_particulas();
};

prepara_html = function() {
  var cmap_nome, i, j, len, len1, m, ref, ref1, s;
  s = '';
  ref = gr.material_list;
  ...
  s = '';
  ref1 = get_colormap_list();
  for (j = 0, len1 = ref1.length; j < len1; j++) {
    cmap_nome = ref1[j];
    s += '<option>' + cmap_nome + '</option>';
  }
  return document.getElementById('cores').innerHTML = s;
};

renderiza = function() {
  var color_params, i, inst, len, ref, u;
...
define_pipeline_material_das_particulas = function() {
  var mt, pip, url;
  ...
  return ps.set_all_material(mt);
};

upd = function() {
  var cfg, cmap, cmap_nome, hex, i, len, reverte, rgb, s;
  define_pipeline_material_das_particulas();
  cfg = ps.get_config();
  cfg.is_emitting = document.getElementById('emitindo').checked;
  cfg.pos_start = vec(parseFloat(document.getElementById('pos_x').value), parseFloat(document.getElementById('pos_y').value), parseFloat(document.getElementById('pos_z').value));
  cfg.vel_start = vec(parseFloat(document.getElementById('vel_x').value), parseFloat(document.getElementById('vel_y').value), parseFloat(document.getElementById('vel_z').value));
  ...
  cmap = colormap(cmap_nome, {
    reverse: reverte
  });
  cfg.default_per_particle.color_list = cmap;
  s = '';
  for (i = 0, len = cmap.length; i < len; i++) {
    rgb = cmap[i];
    hex = rgb_to_hex(rgb);
    s += '<div style="background:' + hex + '"></div>';
  }
  return document.getElementById('cores_pal').innerHTML = s;
};

tema = function(k) {
  if (k === 0) {
    return document.getElementById('tela').style.background = 'linear-gradient(rgb(255,255,255), rgb(255,255,255))';
  } else if (k === 1) {
    return document.getElementById('tela').style.background = 'linear-gradient(rgb(0,0,0), rgb(0,0,0))';
  } else {
    return document.getElementById('tela').style.background = 'linear-gradient(rgb(150, 133, 183), rgb(69, 58, 60))';
  }
};
